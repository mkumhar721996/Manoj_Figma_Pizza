...
test('saves the relationship and makes it visible on both records', () => {
  ...
  linkAsDuplicate(
    { actingUser: triager, duplicateDefectId: 'dup-1', canonicalDefectId: 'canonical-1' },
    repo
  );

  assert.equal(repo.get('dup-1')?.duplicateOfId, 'canonical-1');
  assert.deepEqual(
    repo.findDuplicatesOf('canonical-1').map((d) => d.id),
    ['dup-1']
  );
});
...
test('rejects the action for an unauthorized user', () => {
  ...
  assert.throws(
    () =>
      linkAsDuplicate(
        { actingUser: reporter, duplicateDefectId: 'dup-1', canonicalDefectId: 'canonical-1' },
        repo
      ),
    UnauthorizedError
  );
...
test('rejects linking when the duplicate defect does not exist', () => {
  const triager = { id: 'u1', role: 'TRIAGER' };

  assert.throws(
    () =>
      linkAsDuplicate(
        { actingUser: triager, duplicateDefectId: 'missing', canonicalDefectId: 'canonical-1' },
        repo
      ),
    NotFoundError
  );
...
test('rejects linking when the canonical defect does not exist', () => {
  const triager = { id: 'u1', role: 'TRIAGER' };

  assert.throws(
    () =>
      linkAsDuplicate(
        { actingUser: triager, duplicateDefectId: 'dup-1', canonicalDefectId: 'missing' },
        repo
      ),
    NotFoundError
  );
